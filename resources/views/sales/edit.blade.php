            ...
            <form action="{{ route('sales.update', $sale->id) }}" method="POST">
                ...
                <div class="form-group col-md-4">
                    ...
                    <select id="member_id" name="member_id" class="form-control" required>
                        ...
                        <option {{ $member->id == $sale->member_id ? 'selected' : ''}} value="{{ $member->id }}">{{ $member->mem_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-4">
                    <label for="sale_date">Sale Date</label>
                    <input id="sale_date" type="date" value="{{ date('Y-m-d', strtotime($sale->sale_date)) }}" name="sale_date" class="form-control" required>
                </div>
                <div class="form-group col-md-4">
                    <label for="payment_method">Payment Method</label>
                    <select id="payment_method" name="payment_method" class="form-control" required>
                        <option {{ $sale->payment_method == 'cash' ? 'selected' : ''}} value="cash">Cash</option>
                        <option {{ $sale->payment_method == 'card' ? 'selected' : ''}} value="card">Card</option>
                    </select>
                </div>
                <div class="form-group col-md-4">
            ...
            <div class="row">
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tbody id="product-items">
                        @foreach($sale->items as $saleDetail)
                        <tr>
                            <td>
                                <select name="product_id[]" onchange="get_price(this)"  class="form-control product_id" required>
                                    <option value="">Select Product</option>
                                    @foreach($products as $product)
                                    <option {{ $product->id == $saleDetail->product_id ? 'selected' : ''}} value="{{ $product->id }}">{{ $product->product_name }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                <input  type="number" name="quantity[]" onkeyup="updateTotal(this)" onchange="updateTotal(this)" value="{{ $saleDetail->quantity }}" class="form-control quantity" min="1" required>
                            </td>
                            <td>
                                <input type="number" name="price[]"  value="{{ $saleDetail->unit_price }}"  class="form-control price" min="0" required readonly>
                            </td>
                            <td>
                                <input type="number" name="total[]" value="{{ $saleDetail->total }}" class="form-control total" min="0" required readonly>
                            </td>
                            <td>
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td><input type="number" id="discount" onchange="updateGrandTotal()" onkeyup="updateGrandTotal()"  name="discount" value="{{ $sale->discount }}" class="form-control discount" min="0"></td>
                        ...
                        <tr>
                            ...
                            <td><input type="number" id="grand_total" name="grand_total" value="{{ $sale->total_amount }}" class="form-control grand_total" min="0" required readonly"></td>
                        </tr>
                    </tfoot>
                </table>  
            </div>            
